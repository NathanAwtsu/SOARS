...
<center>
    ...
    <div class="card" style="height: auto; width: 700px;">
        ...
        <form action="/rso_list/pending_save/{{$org->id}}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <textarea id="name" name="name" rows="2" cols="50" required>{{$org->name}}</textarea><br><br>
            <label for="OrgName"><h2>Nickame :</h2></label><br>
            <textarea id="nameickname" name="nickname" rows="2" cols="50" required>{{$org->nickname}}</textarea><br><br>
            ...
            <div id="listOfOfficersContent" class="card mt-4 mb-4" style="height: auto;">
                <div class="card-body">
                    ...
                    <br><br>
                        ...
                        <button href="/rso_list" style="background-color: #7e7e7e; color: #fff; padding: 10px 15px; border-radius: 5px; cursor: pointer;margin-right:550px;">Cancel</button>
                    </div>
                </div>
            </div>
        </form>
